...
const exercicios = ['Supino', 'Agachamento', 'Rosca biceps', 'Remada unilateral', 'abdominal'];
    ...
    return(
        <Body>
            ...
            <ModalHeader>
                <Container>
                    <h3>{name}</h3>
                    <span onClick = { setOpen }><FaTimes/></span>
                </Container>
            </ModalHeader>
            <ModalBody>
                <Container>
                    ...
                    <select name="select" id="group">
                        {grupoMuscular.map((item, index)=> <option key = {index} value = {item}>{item}</option>)}
                    </select>
                    <DivSelect>
                        <Container>
                            <select name="select" id="exercises">
                                {exercicios.map((item, index)=> <option key = {index} value = {item}>{item}</option>)}
                            </select>
                    ...
                    <ul>
                        {prototipo.map((item, index) =>
                            <li key = {index}>{item.nome}
                                <span>
                                    <FaTrashAlt  onClick = { ()=> console.log('click na lixeira') }/>
                                </span>
                            </li>
                        )}
                    </ul>
